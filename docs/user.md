...
* **Image**: will use the following fields in order, if it's available: thumbnail of document, cover of document, gallery images. Appended will be the gravatar email (or the author picture, if gravatar is not configured).
...
|disableSiteImageMetaTags|Set to `true` to disable the profile or gravatar image meta tag. By default, if the posts have a coverImage, thumbnailImage or imageGallery, there is a `<meta property="og:image">` and `<meta property="twitter:image">` tag for each of them, in addition to the one generated by either the profile or the gravatar picture.|
